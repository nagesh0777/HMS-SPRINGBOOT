...
import {
    ...
    AlertTriangle, Wrench, Copy, Lock, RefreshCw, CheckCircle, QrCode
} from 'lucide-react';
...
const Toast = ({ message, type, onClose }) => (
    <motion.div
        ...
        className={`fixed bottom-20 right-4 md:bottom-6 md:right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl text-sm font-bold ${type === 'success' ? 'bg-green-600 text-white' :
...
const DoctorManagementPage = () => {
    ...
    const handlePhotoChange = async (e) => {
        ...
        if (editingId) {
            ...
            try {
                ...
                uploadData.append('type', 'doctor');
                uploadData.append('id', editingId);
                ...
                if (res.data.Status === 'OK') {
                    ...
                    showToast(res.data.ErrorMessage || "Upload failed.", "error");
                }
            } catch (err) {
    ...
    const handleQrChange = async (e) => {
        ...
        if (editingId) {
            ...
            try {
                const uploadData = new FormData();
                uploadData.append('file', file);
                uploadData.append('type', 'doctor_qr');
                uploadData.append('id', editingId);
                const res = await axios.post('/api/Files/UploadPhoto', uploadData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                if (res.data.Status === 'OK') {
                    ...
                    showToast("Consultation QR code updated successfully.");
                } else {
                    showToast(res.data.ErrorMessage || "Upload failed.", "error");
                }
            } catch (err) {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (!editingId && selectedPhotoFile && res.data.Results?.doctor?.doctorId) {
            ...
            if (!editingId && selectedQrFile && res.data.Results?.doctor?.doctorId) {
                const newId = res.data.Results.doctor.doctorId;
                const uploadData = new FormData();
    ...
    const autoUsername = form.fullName
        ? (form.fullName.split(' ')[0].toLowerCase().replace(/[^a-z0-9]/g, '') || 'doctor')
        : '';
                    ...
                    >
                        <form onSubmit={handleSave} className="rounded-2xl bg-white shadow-lg ring-1 ring-gray-100 p-6 space-y-5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {/* Photo Upload Widget */}
                                <div className="flex flex-col items-center justify-center p-6 border border-dashed border-gray-200 rounded-2xl bg-gray-50/50 hover:bg-gray-50 transition-all gap-4">
                                    <div className="relative group w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-md bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-3xl">
                                        {uploading ? (
                                            <div className="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-xs font-bold font-sans">Uploading...</div>
                                        ) : null}
                                        {photoPreviewUrl || form.photoPath ? (
                                            <img src={photoPreviewUrl || form.photoPath} alt="Doctor Preview" className="w-full h-full object-cover" />
                                        ) : (
                                            <User size={40} className="text-gray-400" />
                                        )}
                                    </div>
                                    <div className="text-center">
                                        <label className="cursor-pointer px-4 py-2 bg-white border border-gray-300 rounded-xl text-xs font-bold text-slate-700 shadow-sm hover:bg-gray-50 transition-all">
                                            {uploading ? 'Uploading...' : (form.photoPath || photoPreviewUrl ? 'Change Profile Photo' : 'Upload Profile Photo')}
                                ...
                                <div className="flex flex-col items-center justify-center p-6 border border-dashed border-gray-200 rounded-2xl bg-gray-50/50 hover:bg-gray-50 transition-all gap-4">
                                    <div className="relative group w-24 h-24 rounded-2xl overflow-hidden border-4 border-white shadow-md bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-3xl">
                                        {uploadingQr ? (
                                            <div className="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-xs font-bold font-sans">Uploading...</div>
                                        ) : null}
                                        {qrPreviewUrl || form.consultationQrPath ? (
                                            <img src={qrPreviewUrl || form.consultationQrPath} alt="QR Code Preview" className="w-full h-full object-contain bg-white" />
                                        ) : (
                                    ...
                                    <div className="text-center flex flex-col items-center gap-2">
                                        <div className="flex gap-2">
                                            <label className="cursor-pointer px-4 py-2 bg-white border border-gray-300 rounded-xl text-xs font-bold text-slate-700 shadow-sm hover:bg-gray-50 transition-all">
                                                {uploadingQr ? 'Uploading...' : (form.consultationQrPath || qrPreviewUrl ? 'Change QR Image' : 'Upload QR Image')}
                                            ...
                                            {(form.consultationQrPath || qrPreviewUrl) && (
                                                <button
                                                    ...
                                                    className="px-3 py-2 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 rounded-xl text-xs font-bold transition-all"
                                                >
                                                    Remove
                                                </button>
                                            )}
                                        </div>
                                        <p className="text-[10px] text-gray-400 font-medium">JPEG or PNG, maximum 5MB</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Operational Status</label>
                                                    ...
                                                    {(() => {
                                                        ...
                                                        if (first) {
                                                            if (last) {
                                                                ...
                                                                options.push(`${first}_${last}`);
                                                                options.push(`${first}${last}`);
                                                            } else {
                                                                options.push(first);
                                                                options.push(`${first}123`);
                                                            }
                                                        }
                                                        return [...new Set(options)].filter(o => o !== form.userName).map(opt => (
                                                            <button
                                                                key={opt}
                                                                type="button"
                                                                onClick={() => setForm(p => ({ ...p, userName: opt }))}
                                                                className="px-2 py-1 rounded bg-blue-50 hover:bg-blue-100 border border-blue-100 text-[10px] font-bold text-blue-600 transition-colors"
                                                            >
                                                                {opt}
                                                            </button>
                                                        ));
                    ...
                    {filteredDoctors.map((doc, i) => (
                        ...
                        >
                            ...
                            <div className={`px-5 py-4 flex items-center gap-4 ${doc.isActive ? 'bg-gradient-to-r from-blue-50 to-indigo-50' : 'bg-gray-50'}`}>
                                <div className={`w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center text-lg font-black text-white ${doc.isActive ? 'bg-gradient-to-br from-blue-500 to-indigo-600' : 'bg-gray-400'} flex-shrink-0 shadow-sm border border-white/50`}>
                                    {doc.photoPath ? (
                                        <img src={doc.photoPath} alt={doc.fullName} className="w-full h-full object-cover" />
                                    ) : (
                                ...
                                <div className="flex flex-col items-end gap-1">
                                    ...
                                    {doc.status && (
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                                            doc.status === 'On Duty' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                                            doc.status === 'On Leave' ? 'bg-amber-100 text-amber-800 border border-amber-200' :
                                    ...
                                    {doc.employeeId ? (
                                        ...
                                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-600 border border-amber-100">
