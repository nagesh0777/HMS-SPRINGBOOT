...
import { Users, Calendar, Bed, Download, Building, CheckCircle, XCircle, IndianRupee, Sparkles, RefreshCw } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-200 pb-5">
                <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">Live analytics from your hospital database.</p>
                </div>
            </div>
            ...
            {true && (
                <>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {analytics.patients.slice(0, 6).map(p => (
                                            <tr key={p.patientId} onClick={() => navigate(`/dashboard/patients/${p.patientId}`)} className="border-b border-gray-55 hover:bg-blue-55/30 cursor-pointer transition-colors">
                                                ...
                                                <td className="py-2.5 px-3 text-xs text-gray-505">{p.phoneNumber || '-'}</td>
                    ...
                    {/* AI Groq Insights — only shown when generated from real data */}
                    {aiInsights.length > 0 && (
                        <div className="rounded-2xl bg-gradient-to-br from-indigo-900 to-slate-900 p-6 text-white shadow-xl ring-1 ring-white/10">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-base font-extrabold flex items-center gap-2">
                                    <Sparkles size={18} className="text-yellow-400" />
                                    Groq AI Hospital Insights
                                </h3>
                                <button onClick={fetchAIInsights} disabled={generatingInsights}
                                    className="flex items-center gap-1 text-[10px] font-bold bg-white/10 px-3 py-1.5 rounded-lg hover:bg-white/20 transition-all text-indigo-200 border border-white/5">
                                    <RefreshCw size={11} className={generatingInsights ? 'animate-spin' : ''} />
                                    {generatingInsights ? 'Analyzing...' : 'Re-Analyze'}
                                </button>
                            </div>
                            <div className="space-y-3">
                                {aiInsights.map((ins, i) => (
                                    <div key={i} className="flex items-start gap-3 bg-white/5 p-3 rounded-xl border border-white/5">
                                        <div className="h-5 w-5 rounded-md bg-yellow-400/20 text-yellow-300 flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5">{i + 1}</div>
                                        <p className="text-xs text-slate-200 leading-relaxed font-medium">{ins}</p>
                                    </div>
                                ))}
                            </div>
                            <div className="border-t border-white/10 pt-3 mt-4 text-[10px] font-bold text-slate-400 flex justify-between items-center">
                                <span>Engine: LLaMA-3.3-70B via Groq Cloud — analysis based on your live hospital data</span>
                            </div>
                        </div>
                    )}
                    {aiInsights.length === 0 && full && (
                        <div className="rounded-2xl bg-gradient-to-br from-indigo-900 to-slate-900 p-6 text-white ring-1 ring-white/10">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="text-base font-extrabold flex items-center gap-2 mb-1">
                                        <Sparkles size={18} className="text-yellow-400" />
                                        Groq AI Hospital Insights
                                    </h3>
                                    <p className="text-xs text-slate-400">Generate intelligent insights based on your real hospital data — revenue trends, patient patterns, and operational signals.</p>
                                </div>
                                <button onClick={fetchAIInsights} disabled={generatingInsights}
                                    className="flex-shrink-0 ml-6 flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black rounded-xl transition-all shadow-lg active:scale-95">
                                    <Sparkles size={13} />
